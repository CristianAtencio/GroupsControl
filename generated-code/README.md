...
This compiles the client into a jar located at `./target/integracionGroupsControl-sdk-1.0.0.jar`. Note that this jar does not include any dependencies.
...
```perl
dependencies {
    compile 'api-integracionGroupsControl:integracionGroupsControl-sdk:1.0.0'
}
```
...
<dependencies>
    <dependency>
        <groupId>api-integracionGroupsControl</groupId>
        <artifactId>integracionGroupsControl-sdk</artifactId>
...
This will compile the client and package all dependencies into a jar located at `./target/integracionGroupsControl-sdk-1.0.0.jar`.
...
```java
IntegracionGroupsControlClientBuilder builder = IntegracionGroupsControl.builder();
```
...
```java
IntegracionGroupsControl client = IntegracionGroupsControl.builder()
...
```java
IntegracionGroupsControl client = IntegracionGroupsControl.builder().build();
client.shutdown();
...
## Making requests
After a client is configured and created, you can make a request to the service. A method on the client interface (`IntegracionGroupsControl`) is created for all actions (resource + method) in your API.
...
```java
IntegracionGroupsControl client = IntegracionGroupsControl.builder().build();
PostGroupsResult result = client.postGroups(new PostGroupsRequest());
```
...
```java
IntegracionGroupsControl client = IntegracionGroupsControl.builder().build();
client.postGroups(new PostGroupsRequest().sdkRequestConfig(
...
Service exceptions and client exceptions can be handled separately. Any exceptions modeled in the API will be a subtype of IntegracionGroupsControlException.
```java
try {
    client.postGroups(...);
} catch(IntegracionGroupsControlException e) {
   // All service exceptions will extend from IntegracionGroupsControlException.
   // Any unknown or unmodeled service exceptions will be represented as a IntegracionGroupsControlException.
